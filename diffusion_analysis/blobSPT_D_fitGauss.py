...
for ii, sample in enumerate(samples):
    ...
    data = all_data[all_data['blob_class']==sample]
   ...
   # dapps = data["D"].to_numpy().reshape(-1,1)
    ...
    x = np.linspace(np.log10(1*10**-4), np.log10(5*10**1), 10000)#, min(dapps), max(dapps), 1000000)
    ...
    if ii == 0:
        line = axes.plot(x, pdf, '-', linewidth=3, color=colors[0])
    else:
        line = axes.plot(x, pdf, '--', linewidth=3, color=colors[0])
        
    #indi_line = ax[1].plot(x, pdf_individual, '--k', linewidth=3)
   
    axes.set_ylim(0,1.3)
    axes.set_xlim(np.log10(1*10**-4), np.log10(5*10**1))
