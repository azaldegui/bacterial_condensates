...
def focus_detect(mask, img, file_id, label, rep_no, t, plotfig=False):
    ...
    for kk, cell in enumerate(cells):
        ...
        if cell_width <= 22 and cell_width >= 12 and cell_length >= 20: 
            ...
            if cell_label > 1:# and len(cell_mask_og) > 0:
                ...
                nblobs, blob_mask, cyto_mask = findblobs(cell_fluor, 
                                   mask_hull, cell_label, norm_thresh = 0.65,
...
# for ms datasets
phasemask_files = [file for file in files if "_masks" in file]
...
for ii, file in enumerate(fluor_files[:]):
    ...
    fl_img = read_TIFFStack(file)
    mask = read_TIFFStack(phasemask_files[ii])
    
    out = focus_detect(mask, fl_img[0], ii+1, label, replicate_no, time, plotfig=False)
...
df.columns = ['FILE_ID', 'CELL_ID',  
              'Label', 'Replicate','Time', "Focus", "Cell Intensity", "Cell Area",
              "Cytoplasm Intensity", "Cytoplasm Area", "Focus Intensity", "Focus Area"]
print(df)
