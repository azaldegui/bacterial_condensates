...
axviol = sns.violinplot(ax=axes,x='Sample', y='uM', data=all_data,split=True,
                    zorder=0, cut=0, bw='scott', hue='BlobBool', order = order_,
...
g = sns.stripplot(ax=axes, x='Sample', y='uM', dodge=True,
                  data = all_data, size=4, zorder=1, edgecolor='black', linewidth=0,
                  alpha=0.25, jitter=0.3, hue='BlobBool',order = order_,
                  palette=['gray', 'firebrick'])
    
g.set_xticklabels(['PopTag_LL', 'McdB'])   
...
for ii in range(1):
   ...
   #                   fontsize=font_size)
    ...
    axes.set_ylabel(r'Concentration$_{\rm app}$' + u' (\u03bcM)',fontsize=font_size)
...
for sample in samples:
    ...
    print(protein_df)
    print("classfied", protein_df.groupby('BlobBool')[var_plot].describe())

    print(ttest_ind(protein_df[protein_df['BlobBool']==0]['uM'].astype('float64') , 
                    protein_df[protein_df['BlobBool']==1]['uM'].astype('float64') , equal_var=False))
