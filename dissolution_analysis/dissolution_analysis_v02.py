...
for ii,file in enumerate(tif_files[:]):
    ...
    for jj, image in enumerate(fluor_stack):
        ...
        for kk, cell in enumerate(cells[:]):
            ...
            cell_data.append((sample, ii, cell_label, t, cell_area, total_cell_fluor, avg_cell_fluor))
    ...
    if len(avg_fluor) == 2:
        ...
        cell_data_df.columns = ['sample','file','cell_label', 'time', 'cell_area',
                                'total_cell_fluor','avg_cell_fluor']
...
df.columns = ['sample','file','cell_label', 'time', 'cell_area',
                        'total_cell_fluor','avg_cell_fluor']
print(df)
df.to_csv(directory[:-5] + sample + '_ceph_dissolution.csv', index = False)   
